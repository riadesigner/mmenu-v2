...
class QRCode {
    ...
    function setTypeNumber($typeNumber): void {
    ...
    function setErrorCorrectLevel($errorCorrectLevel): void {
    ...
    function addData($data, $mode = 0): void {
        ...
        match ($mode) {
            QR_MODE_NUMBER => $this->addDataImpl(new QRNumber($data) ),
            QR_MODE_ALPHA_NUM => $this->addDataImpl(new QRAlphaNum($data) ),
            QR_MODE_8BIT_BYTE => $this->addDataImpl(new QR8BitByte($data) ),
            QR_MODE_KANJI => $this->addDataImpl(new QRKanji($data) ),
            default => trigger_error("mode:$mode", E_USER_ERROR),
        };
    }
    ...
    function hex2rgb($hex = 0x0) {
        return ['r' => floor($hex / 65536), 'g' => floor($hex / 256) % 256, 'b' => $hex % 256];
    }
    ...
    function makeImpl($test, $maskPattern): void {
        ...
        for ($i = 0; $i < $this->moduleCount; $i++) {
            $this->modules[] = (new QRCode())->createNullArray($this->moduleCount);
        }
        ...
        $data = (new QRCode())->createData($this->typeNumber, $this->errorCorrectLevel, $dataArray);
    ...
    function createData($typeNumber, $errorCorrectLevel, $dataArray) {
        ...
        return (new QRCode())->createBytes($buffer, $rsBlocks);
    }
    ...
    function createBytes(&$buffer, &$rsBlocks) {
        ...
        $dcdata = (new QRCode())->createNullArray(count($rsBlocks));
        $ecdata = (new QRCode())->createNullArray(count($rsBlocks));
        ...
        for ($r = 0; $r < $rsBlockCount; $r++) {
            ...
            $dcdata[$r] = (new QRCode())->createNullArray($dcCount);
            ...
            $ecdata[$r] = (new QRCode())->createNullArray($rsPoly->getLength() - 1);
        ...
        $data = (new QRCode())->createNullArray($totalCodeCount);
...
class QRUtil {
    ...
    static $QR_PATTERN_POSITION_TABLE = [[], [6, 18], [6, 22], [6, 26], [6, 30], [6, 34], [6, 22, 38], [6, 24, 42], [6, 26, 46], [6, 28, 50], [6, 30, 54], [6, 32, 58], [6, 34, 62], [6, 26, 46, 66], [6, 26, 48, 70], [6, 26, 50, 74], [6, 30, 54, 78], [6, 30, 56, 82], [6, 30, 58, 86], [6, 34, 62, 90], [6, 28, 50, 72, 94], [6, 26, 50, 74, 98], [6, 30, 54, 78, 102], [6, 28, 54, 80, 106], [6, 32, 58, 84, 110], [6, 30, 58, 86, 114], [6, 34, 62, 90, 118], [6, 26, 50, 74, 98, 122], [6, 30, 54, 78, 102, 126], [6, 26, 52, 78, 104, 130], [6, 30, 56, 82, 108, 134], [6, 34, 60, 86, 112, 138], [6, 30, 58, 86, 114, 142], [6, 34, 62, 90, 118, 146], [6, 30, 54, 78, 102, 126, 150], [6, 24, 50, 76, 102, 128, 154], [6, 28, 54, 80, 106, 132, 158], [6, 32, 58, 84, 110, 136, 162], [6, 26, 54, 82, 110, 138, 166], [6, 30, 58, 86, 114, 142, 170]];
...
class QRNumber extends QRData {
    ...
    function write(&$buffer): void {

        $data = $this->getData();

        $i = 0;

        while ($i + 2 < strlen((string) $data) ) {
            $num = QRNumber::parseInt(substr((string) $data, $i, 3) );
            $buffer->put($num, 10);
            $i += 3;
        }
        ...
        if ($i < strlen((string) $data) ) {
            ...
            if (strlen((string) $data) - $i == 1) {
                $num = QRNumber::parseInt(substr((string) $data, $i, $i + 1) );
                $buffer->put($num, 4);
            } else if (strlen((string) $data) - $i == 2) {
                $num = QRNumber::parseInt(substr((string) $data, $i, $i + 2) );
                $buffer->put($num, 7);
            }
        }
    }
...
class QRPolynomial implements Stringable {
...
class QRBitBuffer implements Stringable {
