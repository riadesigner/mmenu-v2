...
register_shutdown_function(function(){
	...
	if (!isset($_FILES['up_file']) || !count($_FILES['up_file'])) __errorjson("Empty file, ".__LINE__);
	...
	if($size===false) __errorjson("allowed image only, ".__LINE__);
	if($size[0]*$size[1]>24000000) __errorjson("the file too big! Required 24Mpx maximum, ".__LINE__);

	if(!App::delete_item_image($item)) __errorjson("cant delete old images, ".__LINE__); 
	...
	if(!$image_large || !$image_small) __errorjson("Can not resized image, ".__LINE__);
...
function get_resized_jpeg($width,$height,$src,$need_rotate){
	...
	if ($size === false) return false;		
  	...
  	if (!function_exists($icfunc)) {
		glog('function not exist '.$icfunc);	
		return false;
	}
	
	$needResize = ($size[0]>$width || $size[1]>$height) ? true : false; 
	
	glog('$format='.print_r($format,1));	
	glog('$needResize='.print_r($needResize,1));	
