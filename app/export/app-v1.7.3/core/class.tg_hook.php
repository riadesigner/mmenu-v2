...
class Tg_hook {
    ...
    function __construct($data){
        ...
        $tg_user_name = sanitizeForDatabase($tg_user_name);
    ...
    public function parse(): void{
        ...
        if($this->REAL_TG_USER){            
            ...
            if($this->CALLBACK_MODE){
                ...
                if($this->check_if_waiting_for_comment_to_order()){
                    // не отвечаем, т.к. ожидаем комментарий к заказу
                    // и принимаем введеный текст от данного официанта (менеджера)
                    // за комментарий к последнему заказу, который взял данный официант
                    $this->close_comment_to_order();
                    return;
                }
    ...
    private function parse_user_callback(): void{
        ...
        switch($command){
            ...
            case 'add_comment':
                $cafe_uniq_name = $params[1];
                $order_id_uniq = $params[2];
                Order_sender::add_comment_to_order($cafe_uniq_name, $order_id_uniq, $this->REAL_TG_USER);
            break;                                  
            default:
    ...
    private function check_if_waiting_for_comment_to_order(): bool{
        $now_typing = !empty($this->REAL_TG_USER->typing_comment_to_order);
        glog("now_typing = $now_typing, ".$this->REAL_TG_USER->typing_comment_to_order);
        return $now_typing;
    }        

    private function close_comment_to_order(): void{
        $order_uniq_id = $this->REAL_TG_USER->typing_comment_to_order;
        $order_short_number = Order_sender::get_short_number($order_uniq_id);        
                         
        // - сбрасываем ожидание коммента от данного официанта
        $this->REAL_TG_USER->typing_comment_to_order = "";
        $this->REAL_TG_USER->save();

        // - добавляем коммент к заказу
        if(!$ORDER = $this->valid_order($order_uniq_id)){                
            $this->send_error_message("Заказ с номером {$order_short_number} ($order_uniq_id) не найден");            
        }else{
            $ORDER->comment = $this->message;
            $ORDER->save();
            $this->send_message("Комментарий к заказу $order_short_number сохранен");
        }
    ...
    private function take_the_order($cafe_uniq_name, $order_uniq_id): void{     
        ...
        $order_short_number = Order_sender::get_short_number($order_uniq_id);
        glog("order_short_number для {$order_uniq_id} = ".$order_short_number);
    ...
    private function create_new_user():void{
        ...
        Попробуйте позже или напишите разработчику сервиса.";
        //xxx
        if(!$this->TG_KEY || !$this->tg_user_id || empty($this->tg_user_name)) {
            $this->send_error_message($error_message);
            return;
        }
