...
export var UVIEWS = {
	...
	set_current:function(name) {
		var _this=this;
		console.log('this.arrViews', this.arrViews);
		if(!this.arrViews[name]){ console.log(`unknown the name ${name}`); return false; };	
		if(name==this.CURRENT_VIEW_NAME){ console.log(`the same name ${name}`); return false; };	
