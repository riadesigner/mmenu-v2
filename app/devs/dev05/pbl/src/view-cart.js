...
export var VIEW_CART = {
	...
	behavior:function() {
		...
		this.$btnOrderingWithWaiter.on("touchend click",(e)=>{
			if(this.READY_TO_ORDERING && !this.chefsmenu.is_loading_now()){				
				if(GLB.MENU_TABLE_MODE.is_table_mode()){
					this.chefsmenu.now_loading();
					this.send_order_to_table();
	...
	update:function(orderId) {
		...
		totalPrice>0?this.$btnOrderingWithWaiter.removeClass(_this.CN+"btn-disabled"):this.$btnOrderingWithWaiter.addClass(_this.CN+"btn-disabled");
