...
if(!PICKUPSELF_MODE){
	$deliv_address = post_clean($order_data["order_user_address"], 250);
	if(empty($deliv_address)) __errorjsonp("--wrong order data, #5");
}else{
	$deliv_address="";	
}
...
foreach ($orders as $row) {		
	...
	$separator = count( $orders ) > $count-1 ? "---------\n" : "--------- //\n";
	$ORDER_TXT .= $separator;
}	
